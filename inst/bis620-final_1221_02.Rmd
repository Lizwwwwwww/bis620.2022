...
setwd("C:/魏珈/Yale/22fall/BIS 620/course_files_export/2022-09-01")
...
Next, we will analyze whether the treatments will have impacts on patients' survival time controlling for the mutant type. 
...
From model4, we know that only the variable `ATRT`is significant in the model, which means that for people with Wild-type mutant on their RAS gene, the treatment of Panitumumab has influence on their survival times.
